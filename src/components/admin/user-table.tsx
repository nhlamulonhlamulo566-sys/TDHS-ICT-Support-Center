...
import { MoreHorizontal, Edit, CheckCircle, XCircle, Trash2, Ban } from "lucide-react"
                      ...
                      <TableCell>
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {!isCurrentUserAdmin && (
                                <>
                                  {user.availability === 'On Leave' && !user.disabled && (
                                    <DropdownMenuItem onClick={() => handleAvailabilityChange(user.id, 'Available')}>
                                      <CheckCircle className="mr-2 h-4 w-4 text-green-500" />
                                      <span>Mark as Available</span>
                                    </DropdownMenuItem>
                                  )}
                                  {user.availability === 'Available' && !user.disabled && (
                              ...
                              {!isCurrentUserAdmin || !isSelf ? (
                                <>
                                  <DropdownMenuItem onSelect={() => setDisablingUser(user)} disabled={isCurrentUserAdmin} className={user.disabled ? "text-green-600 focus:text-green-600" : "text-amber-600 focus:text-amber-600"}>
                                    <Ban className="mr-2 h-4 w-4" />
                                    <span>{user.disabled ? "Enable" : "Disable"} User</span>
                                  </DropdownMenuItem>
                                  <DropdownMenuItem onSelect={() => setDeletingUser(user)} disabled={isCurrentUserAdmin} className="text-destructive focus:text-destructive focus:bg-destructive/10">
                              ...
                              {isCurrentUserAdmin && (
                                <DropdownMenuItem disabled>
                                  <span>Admins cannot be disabled.</span>
                                </DropdownMenuItem>
                              )}
